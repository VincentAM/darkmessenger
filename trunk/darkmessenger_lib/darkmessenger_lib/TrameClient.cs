...
namespace darkmessenger
{
    ...
    public static class TrameClient
    {
        ...
        {
            return "<trame><type>"+TrameType.Message+"</type><from>" + _from + "</from><msg>" + utf8.GetBytes(_msg) + "</msg><to>" + utf8.GetBytes(_to) + "</to></trame>";
        }

        public static string getMsgToAllTrame(string _from, string _msg)
        {
            return "<trame><type>"+TrameType.MessageToAll+"</type><from>" + _from + "</from><msg>" + utf8.GetBytes(_msg) + "</msg></trame>";
        }
